    ...
    Runner.prototype = {
        ...
        update: function () {
            ...
            if (this.playing) {
                ...
                for (var i = 0; i < this.fireballs.length; i++) {
                    ...
                    // Draw fireball (simple red dot)
                    this.canvasCtx.fillStyle = "red";
                    this.canvasCtx.beginPath();
                    this.canvasCtx.arc(f.x, f.y, f.size, 0, Math.PI * 2);
                    this.canvasCtx.fill();

                    // Remove if out of screen
                    if (f.x > this.dimensions.WIDTH) {
                        f.remove = true;
                    }

                    // Collision with first obstacle
                    if (this.horizon.obstacles.length > 0) {
                        var obs = this.horizon.obstacles[0];
                        if (f.x >= obs.xPos &&
                            f.x <= obs.xPos + obs.width &&
                            f.y >= obs.yPos &&
                            f.y <= obs.yPos + obs.typeConfig.height) {
        ...
        onKeyDown: function (e) {
            ...
            if (this.playing && !this.crashed) {
                ...
                if (Runner.keycodes.ATTACK[e.keyCode]) {
                    if (this.fireCooldown <= 0) {
                        this.shootFireball();
